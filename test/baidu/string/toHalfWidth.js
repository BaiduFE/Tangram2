...
test("输入为纯全角字符串", function(){
	...
	equals(baidu.string(strin).toHalfWidth(), strout);
...
test("输入不包含全角的字符串", function(){
	equals(baidu.string("normal en glish").toHalfWidth(), "normal en glish");
	equals(baidu.string("正常字符 串 百度").toHalfWidth(), "正常字符 串 百度");
	equals(baidu.string("百度 n o rmal").toHalfWidth(), "百度 n o rmal");
}); // 5

test("输入包含转义字符的字符串", function(){
	equals(baidu.string("n\ten\\gli\nsh\s").toHalfWidth(), "n\ten\\gli\nsh\s");
}); // 6

test("异常case", function(){
	var nullStr = null;
	equals(baidu.string(nullStr).toHalfWidth(), "null");
	
	var undefinedStr;
	equals(baidu.string(undefinedStr).toHalfWidth(), "undefined");
});
