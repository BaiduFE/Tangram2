...
test('基础测试', function() {
	...
	ok(!baidu.dom(div).hasAttr("rel"), "div has no rel attribute");
	ok(baidu.dom(div).hasAttr("title"), "div has title attribute");
	/* IE下使用class,chrome,FF使用className */
	ok(baidu.dom(div).hasAttr("class") || baidu.dom(div).hasAttr("className"),
...
test('遍历所有元素类型', function() {
	...
	for(var i = 0; i < eleNames.length; i ++){
		...
		ok(baidu.dom(ele).hasAttr('id'), ele.tagName + " has id attribute");
	}
});
...
test('针对一个元素的多个属性进行的判断', function() {
	...
	input.type = "button";
	input.disabled = true;
	ok(baidu.dom(input).hasAttr("type"), "input has type attribute");
	ok(baidu.dom(input).hasAttr("name"), "input has name attribute");
	ok(baidu.dom(input).hasAttr("disabled"), "input has disabled attribute");
});
...
test('针对大写情况下的属性进行判定', function() {
	...
	ok(baidu.dom(div).hasAttr("NEW_ATTRIBUTE"), "new attribute of Uppercase ");
});
...
test('针对特定的默认属性进行测试', function() {
	...
	li.style.color = "blue";
	li.title = "li_title";
	ok(baidu.dom(li).hasAttr("id"), "li has id attribute");
	ok(baidu.dom(li).hasAttr("style"), "li has style attribute");
	ok(baidu.dom(li).hasAttr("title"), "li has title attribute");
});
...
test('特定标签的特定属性测试', function() {
	...
	ok(baidu.dom(a).hasAttr("href"), "a has href attribute");
	ok(baidu.dom(a).hasAttr("name"), "a has name attribute");
	ok(baidu.dom(a).hasAttr("target"), "a has target attribute");
	ok(baidu.dom(a).hasAttr("shape"), "a has shape attribute");
});
