...
test("attr(String)", function() {
	...
	equal( baidu.dom("#text1").attr("type"), "text", "Check for type attribute" );
	equal( baidu.dom("#radio1").attr("type"), "radio", "Check for type attribute" );
	equal( baidu.dom("#check1").attr("type"), "checkbox", "Check for type attribute" );
	equal( baidu.dom("#simon1").attr("rel"), "bookmark", "Check for rel attribute" );
	equal( baidu.dom("#google").attr("title"), "Google!", "Check for title attribute" );
	equal( baidu.dom("#mark").attr("hreflang"), "en", "Check for hreflang attribute" );
	equal( baidu.dom("#en").attr("lang"), "en", "Check for lang attribute" );
	equal( baidu.dom("#simon").attr("class"), "blog link", "Check for class attribute" );
	equal( baidu.dom("#name").attr("name"), "name", "Check for name attribute" );
	equal( baidu.dom("#text1").attr("name"), "action", "Check for name attribute" );
	ok( baidu.dom("#form").attr("action").indexOf("formaction") >= 0, "Check for action attribute" );
	equal( baidu.dom("#text1").attr("value", "t").attr("value"), "t", "Check setting the value attribute" );
	equal( baidu.dom("<div value='t'></div>").attr("value"), "t", "Check setting custom attr named 'value' on a div" );
	equal( baidu.dom("#form").attr("blah", "blah").attr("blah"), "blah", "Set non-existant attribute on a form" );
	equal( baidu.dom("#foo").attr("height"), undefined, "Non existent height attribute should return undefined" );

	// [7472] & [3113] (form contains an input with name="action" or name="id")
	var extras = jQuery("<input name='id' name='name' /><input id='target' name='target' />").appendTo("#testForm");
	equal( baidu.dom("#form").attr("action","newformaction").attr("action"), "newformaction", "Check that action attribute was changed" );
	equal( baidu.dom("#testForm").attr("target"), undefined, "Retrieving target does not equal the input with name=target" );
	equal( baidu.dom("#testForm").attr("target", "newTarget").attr("target"), "newTarget", "Set target successfully on a form" );
	equal( baidu.dom("#testForm").removeAttr("id").attr("id"), undefined, "Retrieving id does not equal the input with name=id after id is removed [#7472]" );
	// Bug #3685 (form contains input with name="name")
	equal( baidu.dom("#testForm").attr("name"), undefined, "Retrieving name does not retrieve input with name=name" );
	extras.remove();

	equal( baidu.dom("#text1").attr("maxlength"), "30", "Check for maxlength attribute" );
	equal( baidu.dom("#text1").attr("maxLength"), "30", "Check for maxLength attribute" );
	equal( baidu.dom("#area1").attr("maxLength"), "30", "Check for maxLength attribute" );

	// using innerHTML in IE causes href attribute to be serialized to the full path
	jQuery("<a/>").attr({ "id": "tAnchor5", "href": "#5" }).appendTo("#qunit-fixture");
	equal( baidu.dom("#tAnchor5").attr("href"), "#5", "Check for non-absolute href (an anchor)" );

	// list attribute is readonly by default in browsers that support it
	jQuery("#list-test").attr("list", "datalist");
	equal( baidu.dom("#list-test").attr("list"), "datalist", "Check setting list attribute" );
	...
	equal( baidu.dom( option ).attr("selected"), "selected", "Make sure that a single option is selected, even when in an optgroup." );
	...
	ok( !!~baidu.dom("#dl").attr("style").indexOf("position"), "Check style attribute getter, also normalize css props to lowercase" );
	ok( !!~baidu.dom("#foo").attr("style", "position:absolute;").attr("style").indexOf("position"), "Check style setter" );
	...
	equal( baidu.dom("#table").attr("test:attrib"), undefined, "Retrieving a non-existent attribute on a table with a colon does not throw an error." );
	equal( baidu.dom("#table").attr("test:attrib", "foobar").attr("test:attrib"), "foobar", "Setting an attribute on a table with a colon does not throw an error." );
	...
	ok( baidu.dom("<div/>").attr("doesntexist") === undefined, "Make sure undefined is returned when no attribute is found." );
	ok( baidu.dom("<div/>").attr("title") === undefined, "Make sure undefined is returned when no attribute is found." );
	equal( baidu.dom("<div/>").attr("title", "something").attr("title"), "something", "Set the title attribute." );
	ok( baidu.dom().attr("doesntexist") === undefined, "Make sure undefined is returned when no element is there." );
	equal( baidu.dom("<div/>").attr("value"), undefined, "An unset value on a div returns undefined." );
	equal( baidu.dom("<input/>").attr("value"), "", "An unset value on an input returns current value." );
});
