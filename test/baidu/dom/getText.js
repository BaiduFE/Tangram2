...
test('getText', function(){
	...
	equal(baidu.dom(text2).getText(),"textNode2","get text from textNode");
	equal(baidu.dom(div).getText(),"textNodetextNode2","div innerHTML is textNodetextNode2");
...
test('dom or id', function(){
	...
	equal(baidu.dom(div).getText(),"text of div","div text");//dom
	equal(baidu.dom("#id_div").getText(),"text of div","div id test");//id
	document.body.removeChild(div);
})
...
test('special character',function(){
	...
	a.innerHTML = "百度一下"
	equal(baidu.dom(a).getText(),"百度一下","text in Chinese");
	a.innerHTML = "^_'{}~@=?|/+-$%&*!<>\();:.,";
	equal(baidu.dom(a).getText(),"^_'{}~@=?|/+-$%&*!<>\();:.,","get special characters");
})
