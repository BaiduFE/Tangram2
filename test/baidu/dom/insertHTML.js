...
test('prepareTest',function(){
	...
	ua.importsrc("baidu.short", function(){
		start();
		ok(true,'ok');
	}, "baidu.trim", "baidu.dom.insertHTML");
});
...
test('在元素的开始标签后插入html', function() {
	...
	oP.innerHTML = 'p2_ih';
	baidu.dom(oP).insertHTML( "afterBegin", "文本信息");
...
test('在元素的结束标签前插入html', function() {
	...
	var oP = insertHtmlCreate('p', "p1_ih", oDiv);
	oP.innerHTML = 'p2_ih';
	baidu(oP).insertHTML("beforeEnd", "END");
...
test('在元素的结束标签后插入html', function() {
	...
	var op1 = baidu.dom(oP).insertHTML( "afterEnd",
...
test('FF下的特殊情况',function(){
	...
	baidu.dom(td).insertHTML( 'beforeEnd', '<div id="test2"><div><table id="table2"><tr><td></td></tr></table></div></div>');
	baidu.dom(td).insertHTML( 'afterBegin', '<div id="test1"><div><table id="table1"><tr><td></td></tr></table></div></div>');
