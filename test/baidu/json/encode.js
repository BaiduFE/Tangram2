...
test("老接口：encode函数输入符合Json要求的对象", function(){
...
test("老接口：输入为number", function(){
	var n = 3.14; //输入为number
	var s = baidu.json.encode(n);
	equals(s, 3.14);
}); // 2

test("老接口：输入为string", function(){
	var n = 'baidu Online'; //输入为string
	var s = baidu.json.encode(n);
	equals(s, "\"baidu Online\"");
}); // 3

test("老接口：输入为null", function(){
	var n = null; //输入为null
	var s = baidu.json.encode(n);
	equals(s, "null");
}); // 4

test("老接口：输入为undefined", function(){
	var n = undefined; //输入为undefined
	var s = baidu.json.encode(n);
	equals(s, "undefined");
}); // 5

test("老接口：输入为boolean", function(){
...
test("老接口：输入的对象当中包含了function", function(){
