...
jQuery.each( [ "", " - new operator" ], function( _, withNew ) {
	...
	function createDeferred( fn ) {
		return withNew ? new baidu.Deferred( fn ) : baidu.Deferred( fn );
	}

	test("baidu.Deferred" + withNew, function() {
...
test( "baidu.Deferred - chainability", function() {
...
test( "baidu.Deferred.then - filtering (done)", function() {
	...
	baidu.Deferred().reject().then(function() {
		ok( false, "then should not be called on reject" );
	});

	baidu.Deferred().resolve().then( jQuery.noop ).done(function( value ) {
		strictEqual( value, undefined, "then done callback can return undefined/null" );
	});
});

test( "baidu.Deferred.then - filtering (fail)", function() {
	...
	baidu.Deferred().resolve().then( null, function() {
		ok( false, "then should not be called on resolve" );
	} );

	baidu.Deferred().reject().then( null, jQuery.noop ).fail(function( value ) {
		strictEqual( value, undefined, "then fail callback can return undefined/null" );
	});
});

test( "baidu.Deferred.then - filtering (progress)", function() {
...
test( "baidu.Deferred.then - deferred (done)", function() {
...
test( "baidu.Deferred.then - deferred (fail)", function() {
...
test( "baidu.Deferred.then - deferred (progress)", function() {
...
test( "baidu.Deferred.then - context", function() {

	expect(4);

	var context = {};

	baidu.Deferred().resolveWith( context, [ 2 ] ).then(function( value ) {
		return value * 3;
	}).done(function( value ) {
...
test( "baidu.when" , function() {
	...
	jQuery.each( {
		...
		ok( jQuery.isFunction( baidu.when( value ).done(function( resolveValue ) {
	...
	ok( jQuery.isFunction( baidu.when().done(function( resolveValue ) {
	...
	baidu.when( baidu.Deferred().resolveWith( context ) ).done(function() {
	...
	jQuery.each([ 1, 2, 3 ], function(k, i) {
		...
		baidu.when( cache || baidu.Deferred( function() {
				this.resolve( i );
			})
		).done(function( value ) {
...
test("baidu.when - joined", function() {

	expect( 119 );

	var deferreds = {
			value: 1,
			success: baidu.Deferred().resolve( 1 ),
			error: baidu.Deferred().reject( 0 ),
			futureSuccess: baidu.Deferred().notify( true ),
			futureError: baidu.Deferred().notify( true ),
			notify: baidu.Deferred().notify( true )
		},
		willSucceed = {
			value: true,
			success: true,
	...
	jQuery.each( deferreds, function( id1, defer1 ) {
		jQuery.each( deferreds, function( id2, defer2 ) {
			...
			baidu.when( defer1, defer2 ).done(function( a, b ) {
				if ( shouldResolve ) {
