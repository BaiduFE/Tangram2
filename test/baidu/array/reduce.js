...
test("基础校验", function() {
	...
	var sum = baidu.array(arr).reduce(ite, 0);
	equals(sum, 6, "校验返回结果");
...
test("特殊情况", function() {
	...
	equals(baidu.array([1]).reduce(function(a, b) {
		// 函数进不来
		ok(false, '只有一个参数');
		return a + b;
	}), 1, '校验返回结果 - 数组只有一项');
...
test('参数判断', function() {
	var step = 0;
	equals(baidu.array([ 0, 1, 2, 3, 4 ]).reduce(function(last, now, index, arr) {
...
test('混合数组，来自w3c示例', function() {
	...
	deepEqual(baidu.array(flatended).reduce(function(last, now) {
		return last.concat(now);
	}), [ 0, 1, 2, 3, 4, 5 ], 'check mix array');
});

test('excluding holes in the array', function() {
	equals(baidu.array([ 0, null, 1 ]).reduce(function(a, b) {
		return a + b;
	}), 1, 'check holes');
	equals(baidu.array([ null, 0, 1 ]).reduce(function(a, b) {
		return a + b;
	}), 1, 'check holes');
	equals(baidu.array([ 0, 1, null ]).reduce(function(a, b) {
		return a + b;
	}), 1, 'check holes');
	equals(baidu.array([ 0, 1, null ]).reduce(function(a, b) {
		return a + b;
