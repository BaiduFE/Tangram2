...
test("2个参数，查询数组项", function() {
	...
	var r = baidu.array(arraytest).indexOf('name');
	equal(r, 4, "index of string");
	arraytest = [ 2, 5, 8, 19, 'name', '44', 56, 5, 'name' ];
	var i = 5;
	r = baidu.array(arraytest).indexOf(i);
	equal(r, 1, "2个参数");
	r = baidu.array(arraytest).indexOf('lack');
	equal(r, -1, "查询不存在的数组项");
	r = baidu.array(arraytest).indexOf(0);
	equal(r, -1, "查询不存在的元素");
});
...
test("3个参数,正常用例", function() {
	...
	var i = 6;
	var start = 5;
	r = baidu.array(arraytest).indexOf(i, start);
	equal(r, 9, "查询数组项");
	r = baidu.array(arraytest).indexOf(i, 2.5);// float
	equal(r, 3, "start为小数");
});
...
test("3个参数，start为负数", function() {
	...
	var i = 6;
	var start = -3;
	var r = baidu.array(arraytest).indexOf(i, start);
	equal(r, 9, "start 为-3");
	start = -15.8;
	r = baidu.array(arraytest).indexOf(i, start);
...
test("3个参数，start大于数组长度", function() {
	...
	var i = 6;
	var start = -3;
	var r = baidu.array(arraytest).indexOf(i, start);// start为负数时自动取为0
	equal(r, 9, "start 为-3");
	start = 15;
	r = baidu.array(arraytest).indexOf(i, start);// start为负数时自动取为数组长度
	equal(r, -1, "查询数组项：start 为15");
	arraytest = [ 2, 3, 5, 6, 7, 3, 1, 6 ];
	start = 20;
	r = baidu.array(arraytest).indexOf(function(x) {
		return x % 3 == 0;
	}, start);
