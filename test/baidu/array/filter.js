...
test("返回数组不为空", function() {
	{
		...
		var rArr = baidu.array(arraytest).filter(fn);
...
test("测试this指针", function() {
	...
	baidu.array(aArray).filter(function(iVal, iIndex) {
...
test("第二个参数不是函数", function() {
	...
	var aArray = [ 1, 2, 3 ];
	var fn = "function";
	var rArr = baidu.array(aArray).filter(fn);
...
test("数组中有未定义的元素", function() {
	...
	var fArray = baidu.array(aArray).filter(fn);
...
test("第一个参数不是数组", function() {
	...
	var fArray = baidu.array(iNumber).filter(fn);
	equal(fArray, "", "对number调用filter");
});

//兼容1.x接口
test("兼容1.x接口：返回数组不为空", function() {
	{
		expect(1);
		var arraytest = [ 1, 2, 3, 4, 6, 7, 8, 9, 11 ];
		var fn = function(x) {
...
test("兼容1.x接口：测试this指针", function() {
	var aArray = [ 1, 2, 3 ], thisObject = {
		a : 'b'
	};
...
test("兼容1.x接口：第二个参数不是函数", function() {
	expect(1);
	var aArray = [ 1, 2, 3 ];
	var fn = "function";
...
test("兼容1.x接口：第二个参数是空json", function() {
	expect(1);
	var aArray = [ 1, 2, 3 ];
	var fn = {};
	var rArr = baidu.array.filter(aArray, fn);
...
test("兼容1.x接口：数组中有未定义的元素", function() {
	expect(1);
	var aArray = [ 1, 2, 3 ];
	var fn = function(x) {
		return x % 2 == 0;
...
test("兼容1.x接口：第一个参数不是数组", function() {
	expect(1);
	var iNumber = 10;
	var fn = function() {
		aArray[iIndex] += iIndex;
