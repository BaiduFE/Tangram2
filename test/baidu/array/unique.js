...
test("输入数组有重复元素", function() {
	...
	var arraytest = [ 2, 3, 6, 2, 4, 2, 3, 'name', 4, 6, 'word', 'name', '2',
			'3' ]; // 输入数组有重复元素
	var rArr = baidu.array(arraytest).unique();
	var array = [ 2, 3, 6, 4, 'name', 'word', '2', '3' ];
	ok(ua.isEqualArray(rArr, array), "输入数组有重复项");
	var input = [ 1, 2, 1, 3 ];
	var output = baidu.array(input).unique();
	equal(output.toString(), "1,2,3", "输入数组有重复元素");

	deepEqual([ 'abc', 'hasOwnProperty', 'toString' ], baidu.array(['abc', 'hasOwnProperty', 'toString', 'abc' ]).unique());
	deepEqual([ 123 ], baidu.array([ Number(123), 123 ]).unique());
	deepEqual([ '123' ], baidu.array([ String(123), String('123'), '123' ]).unique());
});
