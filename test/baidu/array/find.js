...
test("只有一个元素满足", function() {
	...
	r = baidu.array(arraytest).find(function(x) {
		return x == 'odd';
	});
...
test("回调函数中途返回", function() {
	...
	r = baidu.array(arraytest).find(fn);
	equal(r, 3, "回调函数中途返回");
});
...
test("返回结果为空", function() {
	...
	var r = baidu.array(arraytest).find(function(x) {
		if (typeof (x) == 'string')
			return false;
		return x > 15;
	});
...
test("异常case", function() {
	expect(2);
	var r = baidu.array([ 1, 2, 3 ]).find("find function");
	equal(r, null, "not a function");
	var scope = 10;
	r = baidu.array([ 1, 2, 3 ]).find(100, scope);
